...
{
    //16-bit psds for now
    public class OriginalImage(string imagePath, DateTime lastModified, IMagickImage<ushort> background, List<StaticLayer> staticElements, List<Frame> frames)
    {
        public string ImagePath { get; } = imagePath;
        public DateTime LastModified { get; } = lastModified;

        public IMagickImage<ushort> BackgroundMagick { get; } = background;
